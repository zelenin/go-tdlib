...
`, tdlibFunctionReturn.ToGoType()))
		...
		buf.WriteString("\n")
		buf.WriteString(fmt.Sprintf("// %s %s", tdlibFunction.ToGoName(), firstLower(function.Description)))
		buf.WriteString("\n")
		if function.IsSynchronous {
			buf.WriteString(fmt.Sprintf("// Deprecated: %s it is synchronous.", tdlibFunction.ToGoName()))
			buf.WriteString("\n")
		}
		...
		if function.IsSynchronous {
			...
			buf.WriteString(fmt.Sprintf("func (client *Client) %s(ctx context.Context, %s) (%s, error) {\n", tdlibFunction.ToGoName(), requestArgument, tdlibFunctionReturn.ToGoReturn()))

			if len(function.Properties) > 0 {
				buf.WriteString(fmt.Sprintf(`result, err := client.Send(ctx, Request{
	meta: meta{
		Type: "%s",
	},
	Data: map[string]interface{}{
